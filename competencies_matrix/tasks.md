...
**Версия Документа:** 1.9 *(Обновлено)*
**Дата:** [2025-04-22] *(Обновлено)*
...
*   **Задача 1:** **Реализация и Верификация Импорта АУП из Excel.** *(Переформулировано и детализировано)*
    *   **Статус:** [-] **К выполнению/Проверке**.
    *   **Приоритет:** **Критический**.
    *   **Детали:**
        *   [ ] **Добавить CLI команду:** Реализовать или убедиться в наличии Flask CLI команды `flask import-aup --filepath <path> [--force] [--fill-null-modules]` в `app.py` для импорта АУП из Excel (использовать логику из `maps/logic/read_excel.py`, `maps/logic/excel_check.py`, `maps/logic/save_excel_data.py`).
            *   **Файлы для изменений:** `app.py` (добавить CLI команду, если ее нет).
            *   **Используемые файлы:** `maps/logic/read_excel.py`, `maps/logic/excel_check.py`, `maps/logic/save_excel_data.py`.
        *   [ ] **Тестирование импорта:** Запустить команду `flask import-aup` с предоставленным файлом `03 - 000020763 - 2024 Веб-технологии....xlsx` и опцией `--force` (для перезаписи, если АУП уже существует).
        *   [ ] **Проверка валидации:** Убедиться, что логика валидации в `maps/logic/excel_check.py` корректно обрабатывает структуру файла (два листа, заголовки) и сообщает об ошибках, если они есть.
        *   [ ] **Проверка сохранения:** Убедиться, что логика сохранения в `maps/logic/save_excel_data.py` корректно создает запись `AupInfo` в `tbl_aup` и соответствующие записи `AupData` для всех строк из 'Лист2', включая корректное определение/создание ID в справочниках (`d_*`, `spr_*`, `groups`).
        *   [ ] **Верификация в БД:** После импорта вручную проверить наличие `AupInfo` и корректное количество `AupData` для `num_aup = '000020763'` в базе данных.
        *   [ ] **Верификация через API/UI:** Убедиться, что после импорта:
            *   Новый АУП (`000020763`) доступен для выбора в UI Матрицы.
            *   `GET /api/competencies/matrix/000020763` возвращает `200 OK` и список дисциплин, соответствующий Excel-файлу.
*   **Задача 2:** **Финализация и Тестирование CRUD API и UI для Справочников (ОП, ПК/ИПК).**
    ...
    *   **Детали:**
        *   [ ] **Backend:** Протестировать все эндпоинты (`GET`, `POST`, `PATCH`, `DELETE`) для `/programs`, `/competencies`, `/indicators` с фокусом на ПК/ИПК. Проверить валидацию.
            *   **Файлы для изменений:** `competencies_matrix/routes.py`, `competencies_matrix/logic.py`.
        *   [ ] **Frontend:** Протестировать UI (`CompetenciesAdminView.vue` или аналог) для CRUD ОП, Компетенций и ИДК (особенно ПК/ИПК).
            *   **Файлы для изменений:** `cabinet_frontend/src/views/competencies/CompetenciesAdminView.vue` (или подобный).
*   **Задача 3:** **Финализация и Тестирование Загрузки ПС.**
    ...
    *   **Детали:**
        *   [ ] **Backend:** Убедиться, что `POST /profstandards/upload` стабильно работает с реальными файлами HTML/DOCX, корректно вызывает парсер (`profstandard-lean.py` / `parsers.py`) и сохраняет Markdown и метаданные ПС.
            *   **Файлы для изменений:** `competencies_matrix/routes.py`, `competencies_matrix/parsers.py` (или `profstandard-lean.py`).
        *   [ ] **Frontend:** Протестировать UI для загрузки файла ПС.
             *   **Файлы для изменений:** `cabinet_frontend/src/views/competencies/ProfstandardsView.vue` (или подобный).
*   **Задача 4:** **Финализация и Тестирование Матрицы (API + UI).**
    ...
    *   **Детали:**
        *   [ ] **Backend:** Проверить и исправить сортировку/фильтрацию в `GET /matrix/<aup_id>`. Убедиться, что отображаются нужные ПК (например, связанные с ОП).
            *   **Файлы для изменений:** `competencies_matrix/logic.py`, `competencies_matrix/routes.py`.
        ...
        *   [ ] **Frontend:** Проверить UI выбора ОП/АУП, отображение матрицы, индикаторы загрузки, обработку ошибок.
            *   **Файлы для изменений:** `cabinet_frontend/src/components/MatrixView.vue` (или подобный), `cabinet_frontend/src/stores/competenciesMatrix.ts`, `cabinet_frontend/src/services/api/competenciesApi.ts`.
*   **Задача 5:** **Финализация Разграничения Прав Доступа.**
    ...
    *   **Детали:**
        *   [ ] Проверить применение декораторов (`@admin_only`, `@approved_required`) или логики проверки ролей ко всем необходимым API эндпоинтам.
             *   **Файлы для изменений:** `competencies_matrix/routes.py`, `auth/logic.py`.
...
*   **Задача 6:** **Подготовка Демонстрации.**
    ...
    *   **Детали:**
        ...
        *   [ ] Убедиться, что приложение стабильно работает на локальной машине с подготовленными данными (включая импортированный `000020763`).
