...
def _create_uk_indicators_disposition_prompt(disposition_text: str) -> str:
    """
    (НОВАЯ ФУНКЦИЯ)
...
- `applies_to.direction_code`: Extract the exact direction code (e.g., "09.03.01"). If specific codes are not listed, and it says "educational programs of bachelor's degree", put `null` for `direction_code` and fill `education_level`.
- `applies_to.education_level`: Extract the education level (e.g., "бакалавриат", "специалитет", "магистратура"). This is mandatory.
...
def parse_uk_indicators_disposition_with_gemini(disposition_text: str) -> Dict[str, Any]:
    """
    (НОВАЯ ФУНКЦИЯ)
    ...
    if not gemini_client:
        if not GOOGLE_GENAI_SDK_AVAILABLE:
            ...
            raise RuntimeError("Gemini SDK (google-genai) is not available. Cannot parse disposition with NLP.")
        elif not GOOGLE_AI_API_KEY:
            logger.error("GOOGLE_AI_API_KEY environment variable is not set.")
            raise RuntimeError("GOOGLE_AI_API_KEY environment variable is not set. Cannot parse disposition with NLP.")
        else:
            logger.error("Gemini API client model was not initialized (likely due to configuration error).")
            raise RuntimeError("Gemini API client model was not initialized. Cannot parse disposition with NLP.")

    try:
        prompt_content = _create_uk_indicators_disposition_prompt(disposition_text)
        ...
        # --- ГАРАНТИЯ ПРЕОБРАЗОВАНИЯ ДАТЫ для disposition_metadata.date ---
        if 'disposition_metadata' in parsed_data and isinstance(parsed_data['disposition_metadata'], dict):
            disposition_date_value = parsed_data['disposition_metadata'].get('date')
            if isinstance(disposition_date_value, str):
                parsed_date_obj = parse_date_string(disposition_date_value)
                if parsed_date_obj:
                    parsed_data['disposition_metadata']['date'] = parsed_date_obj
                    logger.info(f"Successfully parsed disposition 'date' from string '{disposition_date_value}' to date object: {parsed_date_obj}")
                else:
                    logger.error(f"Could not parse disposition 'date' string '{disposition_date_value}' from Gemini output. Setting to None.")
                    parsed_data['disposition_metadata']['date'] = None
            elif isinstance(disposition_date_value, datetime):
                parsed_data['disposition_metadata']['date'] = disposition_date_value.date()
                logger.info(f"Converted disposition 'date' from datetime '{disposition_date_value}' to date object: {parsed_data['disposition_metadata']['date']}")
            elif not isinstance(disposition_date_value, datetime.date) and disposition_date_value is not None:
                logger.error(f"Disposition metadata 'date' has unexpected type: {type(disposition_date_value)}. Value: {disposition_date_value}. Setting to None.")
                parsed_data['disposition_metadata']['date'] = None
        else:
             if 'disposition_metadata' not in parsed_data: parsed_data['disposition_metadata'] = {}
             parsed_data['disposition_metadata']['date'] = None # Ensure it exists if parsing failed
