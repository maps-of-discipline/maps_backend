...
aup_info_router = Blueprint("aup_info_router", __name__, url_prefix="/api/aup_info", static_folder="static")
...
@aup_info_router.route('/<int:aupcode>/mark_deleted', methods=['POST'])
def mark_aup_deleted(aupcode: int):
    data = request.get_json()
    if not data or 'is_delete' not in data:
        return jsonify({"status": "is_delete parameter is required"}), 400
    is_delete = data['is_delete']
    aup_record: AupInfo = AupInfo.query.filter_by(num_aup=str(aupcode)).first()
    if not aup_record:
        return jsonify({"status": "not found"}), 404
    aup_record.is_delete = 1 if is_delete else 0
    aup_record.date_delete = datetime.now() if is_delete else None
    db.session.add(aup_record)
    try:
        db.session.commit()
    except Exception as ex:
...
def all_deleted_aup():
    aups = db.session.query(
        ...
        NameOP, NameOP.id_spec == AupInfo.id_spec
    ).filter(
        AupInfo.is_delete == 1
    ).all()
    result = [{
