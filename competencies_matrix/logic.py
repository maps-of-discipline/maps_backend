...
def get_program_details(program_id: int) -> Optional[Dict[str, Any]]:
    ...
    try:
        program = EducationalProgram.query.options(
            ...
            selectinload(EducationalProgram.aup_assoc).selectinload(EducationalProgramAup.aup), # Load AupInfo
        ...
        if program.aup_assoc:
            for assoc in program.aup_assoc:
                aup_dict = {}
                if assoc.aup: # assoc.aup is LocalAupInfo from maps.models
                    aup_dict = assoc.aup.to_dict()
                    aup_dict['id_aup'] = assoc.aup.id_aup # Ensure id is present
                    aup_dict['num_aup'] = assoc.aup.num_aup
                else:
                    aup_dict = None
                
                details['aup_list'].append(
                    assoc.to_dict(rules=['-educational_program', '-aup']) | # Merge dictionaries
                    {'aup': aup_dict} 
                )
        ...
        if program.fgos and program.fgos.recommended_ps_assoc:
            sorted_ps_assoc = sorted(program.fgos.recommended_ps_assoc, key=lambda assoc_item: assoc_item.prof_standard.code if assoc_item.prof_standard else '')
            for assoc_item in sorted_ps_assoc:
                if assoc_item.prof_standard:
        ...
        try:
            ...
            for entry in aup_data_entries:
                 result.append({
                     ...
                     'title': entry.discipline, # Use 'discipline' field from ExternalAupData
                     'semester': entry.id_period,
                     'shifr': entry.shifr,
                     'id_type_record': entry.id_type_record,
                     'zet': (entry.zet / 100) if entry.zet is not None else 0, # Divide by 100 if not None
                     'amount': entry.amount,
...
def get_matrix_for_aup(aup_num: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        local_aup_info_entry = session.query(LocalAupInfo).options(
            selectinload(LocalAupInfo.education_programs_assoc) # Use correct relationship name
        ...
        if local_aup_info_entry:
            ...
            if local_aup_info_entry.education_programs_assoc:
                ...
                assoc_to_use = primary_assoc or local_aup_info_entry.education_programs_assoc[0]
        ...
        matrix_response["error_details"] = (matrix_response.get("error_details", "") +
        ...
        if external_aup_search_result["total"] > 0 and external_aup_search_result["items"]:
            ...
            if exact_match_aup:
                ...
                if not local_aup_info_entry:
                    matrix_response["aup_info"] = exact_match_aup # This will be a dict
                ...
                if external_aup_id_for_disciplines is not None:
                    ...
                    matrix_response["error_details"] = (matrix_response.get("error_details", "") + error_msg)
    ...
    if not external_disciplines and local_aup_info_entry:
        ...
        try:
            ...
            if local_aup_data_entries:
                ...
                if matrix_response.get("error_details") and attempted_external_fetch: 
    ...
    if local_aup_info_entry: 
        ...
        if pk_type:
            if educational_program and educational_program.selected_ps_assoc:
                # TODO: Filter PKs based on selected PS for the program. MVP: Load ALL PKs.
                ...
                all_pk_competencies = session.query(Competency).options(
                    selectinload(Competency.indicators), selectinload(Competency.competency_type)
                ).filter(Competency.competency_type_id == pk_type.id).all() # Load all PKs
            ...
            else: # educational_program is None or other conditions for PK loading not met
    ...
    if not local_aup_info_entry and not matrix_response["disciplines"] and matrix_response["source"] != "external_header_only": # If even external_header_only failed
        ...
        if not matrix_response.get("error_details"): # Add error only if not already present
            matrix_response["error_details"] = f"AUP {aup_num} not found in local or external DB, or disciplines could not be loaded."

        # If no AUP info and no disciplines, return None
...
def update_matrix_link(aup_data_id: int, indicator_id: int, create: bool = True) -> Dict[str, Any]:
    ...
    try:
        ...
        # TODO: Check if aup_data_id exists in external DB if needed.
        # MVP: Assume ID is correct from frontend.
        ...
        if create:
            if not existing_link:
                ...
                # If link exists but not manual, make it manual
            ...
            if existing_link:
                ...
                logger.warning(message)
                return { 'success': True, 'status': 'not_found', 'message': message } # Not an error, just info
...
def create_competency(data: Dict[str, Any]) -> Optional[Competency]:
    ...
    try:
        ...
        # Manual POST creation supports PK only (MVP).
        ...
        query = session.query(Competency).filter_by(
             code=str(data['code']).strip(), competency_type_id=comp_type.id
        )
        ...
        # Route will convert SQLAlchemy object to dict
        return competency
...
def update_competency(comp_id: int, data: Dict[str, Any]) -> Optional[Dict[str, Any]]: # Returns a dictionary
    """Updates an existing competency by ID. Returns updated Competency dict on success."""
    ...
    try:
        ...
        # Allowed fields for manual PK update (MVP)
        # Code, type, FGOS/PS links not updatable here.
        allowed_fields = {'name', 'description'} # Use Set for fast lookup
        updated = False
        ...
        for field in data:
            if field in allowed_fields and hasattr(competency, field):
                 ...
                 # Handle None/empty string for description
                 if field == 'description':
                      if processed_value == '': processed_value = None
                      if competency.description == processed_value: continue # No change
                 else:
                     if getattr(competency, field) == processed_value: continue # No change
        ...
        # Convert to dict before returning (as expected by route)
        ...
        raise e # Re-raise for 500 at route level
    except Exception as e:
        session.rollback() 
...
def delete_competency(comp_id: int, session: Session) -> bool:
    ...
    try:
         ...
         logger.info(f"delete_competency: Deletion for {comp_id} prepared in session. Outer commit needed.")
         return True 
         
    except SQLAlchemyError as e:
         session.rollback() 
         logger.error(f"delete_competency: Database error for {comp_id}: {e}", exc_info=True)
         raise e 
    except Exception as e:
         session.rollback() 
         logger.error(f"delete_competency: Unexpected error for {comp_id}: {e}", exc_info=True)
         raise e 
...
def get_all_indicators() -> List[Dict[str, Any]]:
    ...
    try:
        # Load competency for code/name display
...
def create_indicator(data: Dict[str, Any]) -> Optional[Indicator]:
    """Creates a new indicator (IDK) for an existing competency. Returns created Indicator object on success."""
    ...
    try:
        ...
        existing_indicator = session.query(Indicator).filter_by(
             code=str(data['code']).strip(), competency_id=data['competency_id']
        ).first()
...
def update_indicator(ind_id: int, data: Dict[str, Any]) -> Optional[Indicator]:
    """Updates an existing indicator by ID. Returns updated Indicator object on success."""
    logger.info(f"update_indicator: Received update request for indicator ID: {ind_id}. Data: {data}")
    if not data:
        logger.warning(f"update_indicator: No data provided for update for indicator ID {ind_id}.")
        return None
    ...
    try:
            ...
            if field in allowed_fields and hasattr(indicator, field):
                 ...
                 if field == 'source':
                      ...
                      if processed_value != indicator.code:
                           ...
                           if existing_with_new_code:
                                logger.warning(f"update_indicator: Indicator with code {processed_value} already exists for competency {indicator.competency_id}.")
                                raise ValueError(f"Indicator with code {processed_value} already exists for this competency.")
        ...
        session.rollback() 
        logger.warning(f"update_indicator: Validation error for indicator {ind_id}: {ve}")
        raise ve 
    except IntegrityError as e:
        session.rollback()
        logger.error(f"update_indicator: Database IntegrityError for {ind_id}: {e}", exc_info=True)
        raise e 
    except SQLAlchemyError as e:
        session.rollback()
        logger.error(f"update_indicator: Database error for {ind_id}: {e}", exc_info=True)
        raise e 
    except Exception as e:
        session.rollback()
        logger.error(f"update_indicator: Unexpected error for {ind_id}: {e}", exc_info=True)
        raise e 
...
def delete_indicator(ind_id: int, session: Session) -> bool:
    ...
    try:
         ...
         logger.info(f"delete_indicator: Deletion for {ind_id} prepared in session. Outer commit needed.")
         return True 
         
    except SQLAlchemyError as e:
         session.rollback()
         logger.error(f"delete_indicator: Database error for {ind_id}: {e}", exc_info=True)
         raise e 
    except Exception as e:
         session.rollback()
         logger.error(f"delete_indicator: Unexpected error for {ind_id}: {e}", exc_info=True)
         raise e 
...
def parse_fgos_file(file_bytes: bytes, filename: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise Exception(f"Unexpected error parsing FGOS file '{filename}': {e}")
...
def save_fgos_data(parsed_data: Dict[str, Any], filename: str, session: Session, force_update: bool = False) -> Optional[FgosVo]:
    ...
    fgos_date_obj = metadata.get('order_date') # This is already datetime.date from parser
        ...
        with session.begin_nested(): # Use savepoint
            ...
            for parsed_comp in all_parsed_competencies:
                ...
                if existing_comp_for_fgos:
                     logger.warning(f"save_fgos_data: Competency {comp_code} already exists for FGOS {fgos_vo.id}. Skipping.")
                     continue 
            ...
            if len(recommended_ps_codes) > 0:
                 ...
                 for ps_code in recommended_ps_codes:
                    ...
                    if prof_standard:
                        ...
                        logger.warning(f"save_fgos_data: Recommended PS with code {ps_code} not found in DB. Skipping link.")
                 logger.info(f"save_fgos_data: Queued {linked_ps_count} new recommended PS links.")

        logger.info(f"save_fgos_data: Changes for FGOS ID {fgos_vo.id} prepared. Outer commit required.")
        return fgos_vo

    except IntegrityError as e:
        logger.error(f"save_fgos_data: Integrity error for FGOS from '{filename}': {e}", exc_info=True)
        return None 
    except SQLAlchemyError as e:
        logger.error(f"save_fgos_data: Database error for FGOS from '{filename}': {e}", exc_info=True)
        return None
    except Exception as e:
        logger.error(f"save_fgos_data: Unexpected error for FGOS from '{filename}': {e}", exc_info=True)
        return None
...
def get_fgos_details(fgos_id: int) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not fgos:
            logger.warning(f"get_fgos_details: FGOS with id {fgos_id} not found.")
            return None
        ...
        def sort_key_competency(c):
            type_code = comp_types_map_by_id.get(c.competency_type_id, 'ZZZ')
            return (type_code, c.code)
        ...
        for comp in sorted_competencies:
            if comp.competency_type and comp.competency_type.code in ['УК', 'ОПК']: 
                 ...
                 if len(comp_dict['indicators']) > 0:
                      comp_dict['indicators'].sort(key=lambda i: i['code'])
...
def delete_fgos(fgos_id: int, session: Session) -> bool:
    ...
    try:
        with session.begin_nested(): # Start savepoint
        ...
        logger.info(f"delete_fgos: Deletion for FGOS {fgos_id} prepared. Outer commit required.")
        return True 
    ...
    for otf_data in structure_dict.get('generalized_labor_functions', []):
        ...
        for tf_data in otf_data.get('labor_functions', []):
            tf_obj = LaborFunction(
                ...
                generalized_labor_function=otf_obj 
            )
            objects.append(tf_obj)
            for i, la_data in enumerate(tf_data.get('labor_actions', [])):
                 objects.append(LaborAction(labor_function=tf_obj, description=la_data.get('description'), order=i))
            for i, rs_data in enumerate(tf_data.get('required_skills', [])):
                 objects.append(RequiredSkill(labor_function=tf_obj, description=rs_data.get('description'), order=i))
            for i, rk_data in enumerate(tf_data.get('required_knowledge', [])):
                 objects.append(RequiredKnowledge(labor_function=tf_obj, description=rk_data.get('description'), order=i))
    return objects
...
def parse_prof_standard_file(file_bytes: bytes, filename: str) -> Dict[str, Any]:
    ...
    try:
        # parse_prof_standard is in fgos_parser.py
        ...
        if result.get('success') and result.get('parsed_data'):
             ...
             logger.warning(f"parse_prof_standard_file: Parsing failed or no data for {filename}. Error: {result.get('error')}")
             return result 
    except Exception as e:
        logger.error(f"parse_prof_standard_file: Unexpected error during PS parsing for {filename}: {e}", exc_info=True)
        return {"success": False, "error": f"Unexpected error parsing file '{filename}': {e}", "filename": filename, "error_type": "unexpected_error"}
...
def save_prof_standard_data(
    ...
    ps_markdown = parsed_data.get('parsed_content_markdown') # This field is at the top level

    structure_dict = parsed_data.get('structure') 
    if not structure_dict:
        logger.warning(f"save_prof_standard_data: No detailed structure in parsed_data for PS {ps_code}. Saving metadata/markdown only.")
        structure_dict = {} # Ensure _prepare_ps_structure_from_dict doesn't fail

    if not ps_code or not ps_name or ps_markdown is None: 
        logger.error("save_prof_standard_data: Missing essential data (code, name, markdown) from parsed data.")
        return None
        ...
        with session.begin_nested(): # Start savepoint
            ...
            if existing_ps:
                if force_update:
                    ...
                    prof_standard.order_number = parsed_data.get('order_number', prof_standard.order_number)
                    prof_standard.order_date = parsed_data.get('order_date', prof_standard.order_date)
                    prof_standard.registration_number = parsed_data.get('registration_number', prof_standard.registration_number)
                    prof_standard.registration_date = parsed_data.get('registration_date', prof_standard.registration_date)
                    session.add(prof_standard) 
                    session.flush() 
                    logger.info(f"save_prof_standard_data: Existing PS ({prof_standard.id}) updated.")
                else: 
                    logger.warning(f"save_prof_standard_data: PS {ps_code} already exists ({existing_ps.id}). No force update. Skipping.")
                    return existing_ps 
            else: # Create new PS
                ...
                session.add(prof_standard)
                session.flush() 
                logger.info(f"save_prof_standard_data: New ProfStandard created (ID {prof_standard.id}, code {ps_code}).")
            ...
            if structure_dict and isinstance(structure_dict.get('generalized_labor_functions'), list):
            ...
            elif structure_dict and structure_dict.get('generalized_labor_functions') is not None: # Check if it exists but isn't a list
                 ...
                 logger.debug(f"save_prof_standard_data: No detailed structure parsed/provided for PS {ps_code}.")

        logger.info(f"save_prof_standard_data: Final changes for PS ID {prof_standard.id} prepared. Outer commit required.")
        return prof_standard

    except IntegrityError as e:
        message = f"save_prof_standard_data: Integrity error for PS '{ps_code}' from '{filename}': {e}"
        logger.error(message, exc_info=True)
        return None 
    except SQLAlchemyError as e:
        message = f"save_prof_standard_data: Database error for PS '{ps_code}' from '{filename}': {e}"
        logger.error(message, exc_info=True)
        return None
    except Exception as e:
        message = f"save_prof_standard_data: Unexpected error for PS '{ps_code}' from '{filename}': {e}"
        logger.error(message, exc_info=True)
        return None
