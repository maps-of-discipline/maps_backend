...
def save_prof_standard_data(parsed_data: Dict[str, Any], filename: str, session: Session, force_update: bool = False) -> Optional[ProfStandard]:
    ...
    try:
        ...
        logger.error(f"Integrity error saving PS '{ps_code}': {e}", exc_info=True)
        session.rollback()
        raise
    except SQLAlchemyError as e:
        logger.error(f"Database error saving PS '{ps_code}': {e}", exc_info=True)
        session.rollback()
        raise
    except Exception as e:
        logger.error(f"Unexpected error saving PS '{ps_code}': {e}", exc_info=True)
        session.rollback()
        raise
...
def get_prof_standard_details(ps_id: int) -> Optional[Dict[str, Any]]:
    try:
        ...
        logger.error(f"Database error fetching PS {ps_id} details: {e}", exc_info=True)
        return None
    except Exception as e:
        logger.error(f"Unexpected error fetching PS {ps_id} details: {e}", exc_info=True)
        return None
...
def delete_prof_standard(ps_id: int, session: Session) -> bool:
    try:
        ...
        logger.error(f"Database error deleting PS {ps_id}: {e}", exc_info=True)
        raise e
    except Exception as e:
        logger.error(f"Unexpected error deleting PS {ps_id}: {e}", exc_info=True)
        raise e
