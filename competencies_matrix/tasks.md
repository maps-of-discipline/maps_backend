...
*   **Задача 1:** **Финализация Импорта АУП из Excel.**
    ...
    *   **Детали:**
        ...
        *   [ ] **Backend:** Убедиться, что импорт корректно создает/связывает записи в `tbl_aup`, `aup_data` и `EducationalProgram`.
            *   **Файлы для изменений:**
                *   `maps_backend/logic.py`: Проверить и улучшить функцию импорта Excel.
                *   `maps_backend/routes.py`: Проверить API-эндпоинт `/api/competencies/aup/import-excel`.
        *   [ ] **Frontend:** Если используется API, убедиться, что UI для загрузки файла работает корректно.
            *   **Файлы для изменений:**
                *   `cabinet_frontend/src/views/competencies/ImportView.vue` (или подобный): Компонент для загрузки файлов.
...
*   **Задача 2:** **Финализация и Тестирование CRUD API и UI для Справочников (ОП, ПК/ИПК).**
    ...
    *   **Детали:**
        *   [ ] **Backend:** Протестировать все эндпоинты (`GET`, `POST`, `PATCH`, `DELETE`) для `/programs`, `/competencies`, `/indicators` с фокусом на ПК/ИПК. Проверить валидацию.
            *   **Файлы для изменений:**
                *   `maps_backend/routes.py`: Проверить эндпоинты `/programs`, `/competencies`, `/indicators`.
                *   `maps_backend/logic.py`: Проверить функции для операций CRUD.
        *   [ ] **Frontend:** Протестировать UI (`CompetenciesAdminView.vue` или аналог) для CRUD ОП, Компетенций и ИДК (особенно ПК/ИПК).
            *   **Файлы для изменений:**
                *   `cabinet_frontend/src/views/competencies/CompetenciesAdminView.vue`: Проверить UI для CRUD операций.
*   **Задача 3:** **Финализация и Тестирование Загрузки ПС.**
    ...
    *   **Детали:**
        *   [ ] **Backend:** Убедиться, что `POST /profstandards/upload` стабильно работает с реальными файлами HTML/DOCX, корректно вызывает парсер (`profstandard-lean.py` / `parsers.py`) и сохраняет Markdown и метаданные ПС.
            *   **Файлы для изменений:**
                *   `maps_backend/routes.py`: Проверить эндпоинт `/profstandards/upload`.
                *   `maps_backend/parsers.py` или `maps_backend/profstandard-lean.py`: Проверить парсер.
        *   [ ] **Frontend:** Протестировать UI для загрузки файла ПС.
             *   **Файлы для изменений:**
                *   `cabinet_frontend/src/views/competencies/ProfstandardsView.vue` (или подобный): Компонент для загрузки ПС.
*   **Задача 4:** **Финализация и Тестирование Матрицы (API + UI).**
    ...
    *   **Детали:**
        *   [ ] **Backend:** Проверить и исправить сортировку/фильтрацию в `GET /matrix/<aup_id>`. Убедиться, что отображаются нужные ПК (например, связанные с ОП).
            *   **Файлы для изменений:**
                *   `maps_backend/logic.py`: Исправить сортировку/фильтрацию в функции получения данных матрицы.
                *   `maps_backend/routes.py`: Проверить эндпоинт `/matrix/<aup_id>`.
        ...
        *   [ ] **Frontend:** Проверить UI выбора ОП/АУП, отображение матрицы, индикаторы загрузки, обработку ошибок.
            *   **Файлы для изменений:**
                *   `cabinet_frontend/src/components/MatrixView.vue` (или подобный): Компонент матрицы.
                *   `cabinet_frontend/src/stores/competenciesMatrix.ts`: Хранилище Pinia для матрицы.
                *   `cabinet_frontend/src/services/api/competenciesApi.ts`: API-клиент.
*   **Задача 5:** **Финализация Разграничения Прав Доступа.**
    ...
    *   **Детали:**
        *   [ ] Проверить применение декораторов (`@admin_only`, `@approved_required`) или логики проверки ролей ко всем необходимым API эндпоинтам.
             *   **Файлы для изменений:**
                *   `maps_backend/routes.py`: Проверить применение декораторов `@admin_only`, `@approved_required`.
