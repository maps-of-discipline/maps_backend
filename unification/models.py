...
faculty_discipline_period_assoc = db.Table(
    ...
    db.Column(
        "discipline_period_id",
        db.ForeignKey("discipline_period_assoc.id", ondelete="CASCADE"),
    ),
    db.Column(
        "faculty_id", db.ForeignKey("spr_faculty.id_faculty", ondelete="CASCADE")
    ),
)
...
unification_okso_assoc = db.Table(
    ...
    db.Column(
        "unification_id", db.ForeignKey("unification_discipline.id", ondelete="CASCADE")
    ),
    db.Column(
        "okso_id",
        db.String(255),
...
class DisciplinePeriodAssoc(db.Model):
    ...
    unification_discipline_id = db.Column(
        db.Integer, db.ForeignKey("unification_discipline.id", ondelete="CASCADE")
    )
    period_id = db.Column(db.Integer, db.ForeignKey("d_period.id", ondelete="CASCADE"))

    faculties = db.relationship(
        "SprFaculty", secondary=faculty_discipline_period_assoc, lazy="subquery"
    )
    unification_discipline = db.relationship(
        "UnificationDiscipline", back_populates="periods"
    )
...
class UnificationLoad(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    education_form_id = db.Column(
        db.Integer, db.ForeignKey("spr_form_education.id_form", ondelete="CASCADE")
    )
    discipline_period_assoc_id = db.Column(
        db.Integer, db.ForeignKey("discipline_period_assoc.id", ondelete="CASCADE")
    )
...
class UnificationDiscipline(db.Model):
    ...
    measure_id = db.Column(
        db.Integer, db.ForeignKey("d_ed_izmereniya.id", ondelete="CASCADE")
    )

    periods = db.relationship(
        "DisciplinePeriodAssoc",
        lazy="subquery",
        back_populates="unification_discipline",
    )
