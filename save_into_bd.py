...
def SaveCard(db, aupInfo, aupData):
    row = AupInfo(num=aupInfo["num"], type_education=aupInfo["type_education"], degree=aupInfo["degree"], direction=aupInfo["direction"], program_code=aupInfo["program_code"], qualification=aupInfo["qualification"], name_spec=aupInfo["name_spec"] ,type_standard=aupInfo["type_standard"] ,name_faculty=aupInfo["name_faculty"] ,department=aupInfo["department"] ,form_educ=aupInfo["form_educ"], years_begin=aupInfo["years_begin"], period_edication=aupInfo["period_edication"],  base=aupInfo["base"], full_years=aupInfo["full_years"])
    db.session.add(row)
    db.session.commit()
    for i in aupData:
        new_row = AupData(aup_num=row.id, block_id=i[0], shifr=i[1], part_id=i[2], module_id=i[3], type_id=i[4], discipline=i[5], period_control_id=i[6], type_control_id=i[7], amount=int(i[8]), measure=i[9], zet=int(i[10]))
