...
from .parsing_utils import parse_date_string, preprocess_text_for_llm
...
Твоя задача - спарсить предоставленный текст документа ФГОС ВО и извлечь конкретные, структурированные данные в формате JSON.
Строго придерживайся указанной JSON-схемы.

**КЛЮЧЕВЫЕ ПРАВИЛА:**
1.  **ТОЛЬКО JSON:** НЕ ВКЛЮЧАЙ никакого другого текста, объяснений или разметки Markdown за пределами блока JSON. Твой ответ должен быть валидным JSON, обернутым в ```json ... ```.
2.  **ФОРМАТ ДАТЫ:** Все даты в полях `order_date` и `approval_date` ДОЛЖНЫ быть в строгом формате `YYYY-MM-DD`. Например, "25 ноября 2020 г." должно стать `"2020-11-25"`. Это КРИТИЧЕСКИ ВАЖНО.
3.  **ПОЛНОТА СТРУКТУРЫ:** НЕ ОПУСКАЙ никакие поля. Если значение не найдено, используй `null` для строк/дат или `[]` для списков.
4.  **ЭКРАНИРОВАНИЕ:** Убедись, что все строковые значения правильно экранированы для JSON.
