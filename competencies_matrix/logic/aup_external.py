...
from maps.models import (
    ...
    D_Blocks, D_Part, D_Modules, Groups, D_TypeRecord, D_ControlType, D_EdIzmereniya, D_Period,
    SprDiscipline
)

from ..models import EducationalProgramAup, EducationalProgram

from ..external_models import (
    ExtAupInfo, ExternalNameOP, ExternalSprOKCO, ExternalSprFormEducation,
    ExternalSprDegreeEducation, ExtAupData, 
    ExternalSprFaculty,
    ExternalDepartment
)

from ..utils import find_or_create_lookup, find_or_create_name_op
from .error_utils import handle_db_errors, external_db_session
...
def import_aup_from_external_db(aup_num: str, program_id: int, session: Session) -> Dict[str, Any]:
    ...
    program = session.get(EducationalProgram, program_id)
    if not program:
    ...
    try:
        with external_db_session() as external_session:
            external_aup = external_session.query(ExtAupInfo).options(
                joinedload(ExtAupInfo.spec).joinedload(ExternalNameOP.okco),
                joinedload(ExtAupInfo.form), joinedload(ExtAupInfo.degree),
                joinedload(ExtAupInfo.faculty), joinedload(ExtAupInfo.department)
            ).filter_by(num_aup=aup_num).first()
            if not external_aup:
                raise FileNotFoundError(f"АУП с номером '{aup_num}' не найден во внешней базе данных.")

            external_disciplines_query = external_session.query(ExtAupData).options(
                joinedload(ExtAupData.spr_discipline),
                joinedload(ExtAupData.block), joinedload(ExtAupData.part),
                joinedload(ExtAupData.module), joinedload(ExtAupData.type_record_rel),
                joinedload(ExtAupData.type_control_rel), joinedload(ExtAupData.ed_measurements_rel),
                joinedload(ExtAupData.group_rel), joinedload(ExtAupData.period_rel)
            ).filter_by(id_aup=external_aup.id_aup).order_by(
                ExtAupData.id_period, ExtAupData.shifr, ExtAupData.id
            ).all()
    except OperationalError:
        logger.error(f"Не удалось подключиться к внешней БД при импорте АУП '{aup_num}'.")
        raise RuntimeError("Не удалось подключиться к БД 'Карт Дисциплин'.")
    ...
    for ext_disc_data in external_disciplines_query:
        ...
        local_edizm = find_or_create_lookup(D_EdIzmereniya, {'id': ext_disc_data.id_edizm}, {'title': ext_disc_data.ed_measurements_rel.title if ext_disc_data.ed_measurements_rel else f"Ед. изм. {ext_disc_data.id_edizm}"}, session)
...
def get_external_aups_list(
    ...
    try:
        with external_db_session() as session:
            query = session.query(ExtAupInfo).options(
                joinedload(ExtAupInfo.spec).joinedload(ExternalNameOP.okco),
                joinedload(ExtAupInfo.form), joinedload(ExtAupInfo.degree),
                joinedload(ExtAupInfo.faculty), joinedload(ExtAupInfo.department)
            )
            ...
            if year_beg: filters.append(ExtAupInfo.year_beg == year_beg)
            if degree_education_name: filters.append(ExtAupInfo.degree.has(ExternalSprDegreeEducation.name_deg == degree_education_name))

            query = query.join(ExtAupInfo.spec, isouter=True)\
                         .join(ExternalNameOP.okco, isouter=True)
            query = query.join(ExtAupInfo.form, isouter=True)
            query = query.join(ExtAupInfo.degree, isouter=True)
            query = query.join(ExtAupInfo.faculty, isouter=True)
            query = query.join(ExtAupInfo.department, isouter=True)
            ...
            if search_query:
                 ...
                 search_conditions = [
                     ExtAupInfo.num_aup.ilike(search_pattern), ExternalNameOP.name_spec.ilike(search_pattern),
            ...
            query = query.order_by(ExtAupInfo.year_beg.desc(), ExtAupInfo.num_aup)
        ...
        logger.error(f"Не удалось подключиться к внешней БД при получении списка АУП.")
        raise RuntimeError("Не удалось подключиться к БД 'Карт Дисциплин'.")
...
def get_external_aup_disciplines(aup_id: int) -> List[Dict[str, Any]]:
    ...
    try:
        with external_db_session() as session:
            query = session.query(ExtAupData).filter(ExtAupData.id_aup == aup_id)\
                .order_by(ExtAupData.id_period, ExtAupData.shifr, ExtAupData.id)

            query = query.options(
                joinedload(ExtAupData.spr_discipline),
                joinedload(ExtAupData.block),
                joinedload(ExtAupData.part),
                joinedload(ExtAupData.module),
                joinedload(ExtAupData.type_record_rel),
                joinedload(ExtAupData.type_control_rel),
                joinedload(ExtAupData.ed_measurements_rel),
                joinedload(ExtAupData.group_rel),
                joinedload(ExtAupData.period_rel)
            )
        ...
        logger.error(f"Не удалось подключиться к внешней БД при получении дисциплин для АУП ID {aup_id}.")
        raise RuntimeError("Не удалось подключиться к БД 'Карт Дисциплин'.")
    except Exception as e:
